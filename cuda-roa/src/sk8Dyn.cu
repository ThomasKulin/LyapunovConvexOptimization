...
__global__ void simulate(curandState_t* states, double *state, double *initial_state, double *final_state, int slice_x, int slice_y) {
    ...
    for (int t = 0; t < NUM_TIMESTEPS; t++) {
        ...
        // Control Inputs
        int fx = (-0.5 * (10 * ((36.624520729487024 * x + 0.27861843412132048 * phi + 16510.451050942986 * theta - 3.2004724026257469 * psi + 133.76411335652477 * x_dot - 0.20022883155884946 * phi_dot + 1090.6635460300026 * theta_dot - 0.56388233721281655 * psi_dot) * ((-1 * cos(theta)) / (0.90000000000000002 * (4 + 90 * pow(sin(theta), 2))))) + ((10 * (32.433095947025691 * x + 0.031999224782588702 * phi + 1752.320146257649 * theta - 2.1895584321499855 * psi + 86.359313073578377 * x_dot + 0.12290772267221448 * phi_dot + 133.76411335652477 * theta_dot - 2.6298100506676696 * psi_dot)) / (4 + 90 * pow(sin(theta), 2)))));
	int fy = -(-0.5 * (10 * (( - 0.21704087080691076 * x + 106.78915861262998 * phi - 0.3699874871493381 * theta + 61662.985856405969 * psi - 2.6298100506676696 * x_dot + 8.0362227522587357 * phi_dot - 0.56388233721281655 * theta_dot + 8355.1423483685739 * psi_dot) * (cos(psi) / (72.900000000000006 * pow(cos(theta), 2)))) + ((32.433095947025691 * x + 0.031999224782588702 * phi + 1752.320146257649 * theta - 2.1895584321499855 * psi + 86.359313073578377 * x_dot + 0.12290772267221448 * phi_dot + 133.76411335652477 * theta_dot - 2.6298100506676696 * psi_dot) * ((0 / (4 + 90 * pow(sin(theta), 2))) + 10 * ((sin(theta) * sin(psi) * cos(theta)) / (0.90000000000000002 * (4 + 90 * pow(sin(theta), 2)))))) + 10 * ((36.624520729487024 * x + 0.27861843412132048 * phi + 16510.451050942986 * theta - 3.2004724026257469 * psi + 133.76411335652477 * x_dot - 0.20022883155884946 * phi_dot + 1090.6635460300026 * theta_dot - 0.56388233721281655 * psi_dot) * ((-94 * sin(theta) * sin(psi)) / (72.900000000000006 * (4 + 90 * pow(sin(theta), 2)))))));	
	//fx = 0;
	//fy = 0;
	x_dot = xdot;
