...
def sk8dyn_sos_lower_bound(deg, objective="integrate_ring", constraint = "kSos", visualize=False, test=False, actuator_saturate=False, read_file=None):
    ...
    # z = (x, phi, st, ct, sps, cps, xdot, phidot, thetadot, psidot)
    ...
    # z = (x, phi, st, ct, sps, cps, xdot, phidot, thetadot, psidot)
    def f(z, z0, u, T, dtype=Expression):
        phi = z[1]
        st = z[2]
        ct = z[3]
        sps = z[4]
        ...
        # Taylor approximations of sin and cos
        order = 3
        sph = TaylorExpand(sin(z[1]), {z[i]: z0[i] for i in range(nz)}, order)
        cph = TaylorExpand(cos(z[1]), {z[i]: z0[i] for i in range(nz)}, order+1)
        # Ankle torque inputs
        tau_t = (-(Kpprop+Kpv)*st - (Kdprop+Kdv)*theta_dot)
        tau_ps = (-Kpy*(sps-Wprop*phi) - Kdy*(psi_dot-Wprop*phi_dot) - Ky*(sps-phi) - Cy*(psi_dot-phi_dot))  # small angle approximation applied (psi = sps)
        tau_ph = (-tau_ps)
        ...
        f_val[4] = (m2*L*st*(theta_dot**2 + psi_dot**2*ct**2) - g*m2*L*st*ct*cps/L - tau_t*ct/L + u[0]) * denom1 * ry_num  # Denom is m1+m2*st**2  # xddot
        f_val[5] = (tau_ph-Kphi*phi)/Ib * denom2 * denom1 * ry_num  # phiddot
        ...
        f_val[7] = (((2*psi_dot**2*theta_dot**2*st + g*sps/L)*ct + tau_ps/(m2*L**2) - m2*x_dot*u[1]*cps/(m2*L**2))*ry_num - x_dot**2*cps*ry_denom/(L**2)) * denom1  # Denom is ct**2 * ry_num  # psiddot
    ...
    z_max = np.array([1.5, np.pi/4, np.sin(np.pi/2), 1, np.sin(np.pi / 2), 1, 20, 4, 4, 4])
    z_min = np.array([-1.5, -np.pi/4, -np.sin(np.pi/2), 0, -np.sin(np.pi/2), 0, -20, -4, -4, -4])
    ...
    # z = (x, phi, st, ct, sps, cps, xdot, phidot, thetadot, psidot)
    Q_diag = [0.0, 200, 20000, 20000, 20000, 20000, 1, 1, 1, 1]
    ...
    for monomial, coeff in obj.monomial_to_coefficient_map().items():
        s2_deg = monomial.degree(z[2])  # sin(theta)
        c2_deg = monomial.degree(z[3])  # cos(theta)
        s3_deg = monomial.degree(z[4])  # sin(psi)
        c3_deg = monomial.degree(z[5])  # cos(psi)
    ...
    T_val = T(z)
    f_val, denominator = f(z, z0, u, T_val)
    J_dot = J_expr.Jacobian(z).dot(f_val)
    LHS = J_dot + l_cost(z, u) * denominator  # Lower bound on cost to go V >= -l  -->  V + l >= 0
    # LHS = J_dot +   # Relaxed Hamilton jacobian bellman conditions, non-optimal, but still lyapunov
    ...
    # Save data to file
    os.makedirs("Sk8Dyn/data/TaylorPhi/{}/{}".format(z_max, constraint), exist_ok=True)
    save_polynomial(J_star, z, "Sk8Dyn/data/TaylorPhi/{}/{}/J_lower_bound_deg_{}.pkl".format(z_max, constraint, deg))
    uToStr(u_star, "Sk8Dyn/data/TaylorPhi/{}/{}/J_lower_bound_deg_{}.txt".format(z_max, constraint, deg))
