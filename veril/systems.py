...
class ClosedLoopSys(object):
    ...
    def get_sample_variety_features(self, samples):
        ...
        if self.name is "SphericalIP":
            xbar = samples - self.x0
            for i in range(len(xbar)):
                xbar[i, [0,1,4,5]] = 0
        elif self.name is "SphericalIP2":
            xbar = samples - self.x0
            for i in range(len(xbar)):
                xbar[i, [2, 3]] = 0
        else:
    ...
    def sample_stable_inits_concurrent(self, x=None, **kwargs):
        event = self.event
        if x is None:
            x = self.get_x(**kwargs)

        with concurrent.futures.ThreadPoolExecutor() as executor:  # Create thread pool
            # Submit tasks and store futures in order
            futures = {executor.submit(self.forward_sim, i, **kwargs): i for i in x}

            stableSamples = []
            # futures.items ensures results are yielded in the order submitted
            for future, initial_condition in futures.items():
                sol = future.result()
                if sol.status != -1 and self.is_at_fixed_pt(sol.y[:, -1]):
                    stableSamples.append(initial_condition)
    ...
    def sample_stable_inits_concurrent2(self, x=None, **kwargs):
        event = self.event
        if x is None:
            x = self.get_x(**kwargs)

        with Pool() as pool:  # Create a pool of processes
            # Submit tasks and store results in order
            results = pool.map(self.forward_sim_wrapper, [(i, kwargs) for i in x])

        stableSamples = [i for i, res in zip(x, results) if res is not None and self.is_at_fixed_pt(res.y[:, -1])]
    ...
    def forward_sim_wrapper(self, args):
        i, kwargs = args
        return self.forward_sim(i, **kwargs)
...
class SphericalIP2(ClosedLoopSys):
    def __init__(self):
        super().__init__('SphericalIP2', 4, idx=(0, 2))
        self.init_x_f()
        self.degf = 6
        self.int_stop_ub = 40
        self.int_horizon = 15
    ...
    def fx(self, t, x):
        # Taylor3_SDSOS_Q2e4 = np.array([x[4],
        #                  x[5],
        #                  (0.025985759659187119 * 0 + 158.07994174502926 * x[0] - 0.3716336569325065 * x[2] + 6.4587701771726376e-06 * x[3] + 7.5341973451989288 * x[4] + 1.8081490240376541e-05 * x[5] + 8.6101854355325684e-05 * (0 * 0 * x[3]) - 2.2362441675895609e-05 * (0 * x[0] * x[2]) + 2.0126197508306046e-05 * (0 * x[0] * x[4]) - 1.0294480449040951e-05 * (0 * x[0] * x[1]) - 2.219845220968448e-05 * (0 * x[0] * x[3]) + 8.8009424710867011e-05 * (0 * x[2] * x[3]) - 0.00046710617642045367 * (x[0] * x[1] * x[2]) - 0.0002945314534245098 * (x[0] * x[1] * x[3]) - 0.00013254196247309175 * (x[0] * x[1] * x[4]) + 0.013781653511377088 * (x[0] * x[2] * x[4]) - 0.014349195069241701 * (x[1] * x[2] * x[5]) + 0.00017631162648910149 * (x[1] * x[4] * x[5]) + ((-163.21590736129596 * x[0] * pow(x[1], 2)) / 2) + ((-156.93901379606697 * pow(x[0], 3)) / 6) + ((-8.891341671468707 * pow(x[1], 2) * x[4]) / 2) + ((-1.5140437387672692 * pow(x[1], 2) * x[2]) / 2) + ((-0.46981419348525333 * pow(x[2], 3)) / 6) + ((-0.056986517831026637 * x[0] * pow(x[4], 2)) / 2) + ((-0.028514557695434106 * pow(0, 2) * x[0]) / 2) + ((-0.028120879944272436 * pow(0, 2) * x[0]) / 2) + ((-0.025090692738814192 * 0 * pow(x[0], 2)) / 2) + ((-0.016092595799421284 * x[0] * pow(x[3], 2)) / 2) + ((-0.015066799039547983 * x[0] * pow(x[2], 2)) / 2) + ((-0.0073192793988367212 * x[0] * pow(x[5], 2)) / 2) + ((-0.0021314436129659439 * x[2] * pow(x[3], 2)) / 2) + ((-0.00040383728288752351 * pow(0, 2) * x[2]) / 2) + ((-0.00014423248254671469 * pow(0, 2) * x[2]) / 2) + ((-1.0763431740226093e-05 * pow(x[1], 2) * x[5]) / 2) + ((-6.4587701771726376e-06 * pow(x[0], 2) * x[3]) / 2) + ((-6.4587701771726376e-06 * pow(x[1], 2) * x[3]) / 2) + ((7.3180585001504433e-06 * pow(x[0], 2) * x[5]) / 2) + ((8.6355221724694915e-05 * 0 * pow(x[3], 2)) / 2) + ((8.7588547593900578e-05 * 0 * pow(x[5], 2)) / 2) + ((0.0012289624398264913 * 0 * pow(x[2], 2)) / 2) + ((0.0015211431668420085 * pow(x[0], 2) * x[4]) / 2) + ((0.011308141515977189 * 0 * pow(x[1], 2)) / 2) + ((0.029388560621793 * pow(x[0], 2) * x[2]) / 2)),
        #                  (0.028881439833938614 * 0 + 164.31606006519559 * x[1] + 6.4587701771726376e-06 * x[2] - 0.36369515692380849 * x[3] + 1.8081490240376541e-05 * x[4] + 6.4062698640100599 * x[5] - 8.791042233320671e-06 * (0 * 0 * x[0]) + 8.6101854355325684e-05 * (0 * 0 * x[2]) - 0.00011673340560498513 * (0 * x[0] * x[1]) + 8.6355221724694915e-05 * (0 * x[2] * x[3]) - 9.7320608437667311e-05 * (0 * x[2] * x[5]) + 1.9978606988716032e-05 * (0 * x[0] * x[4]) - 0.00013747195370884162 * (0 * x[3] * x[5]) - 1.5100381917224561 * (x[0] * x[1] * x[2]) - 0.0005104843691054417 * (x[0] * x[1] * x[3]) - 8.8929666394410578 * (x[0] * x[1] * x[4]) - 1.8081490240376538e-05 * (x[0] * x[1] * x[5]) + 2.9785274336034646e-05 * (x[0] * x[2] * x[3]) + 0.014510142966381586 * (x[0] * x[3] * x[4]) - 0.0073192793988367212 * (x[0] * x[4] * x[5]) - 0.015541783691952376 * (x[1] * x[3] * x[5]) + ((-657.96028408747316 * pow(x[1], 3)) / 6) + ((-474.91496247076827 * pow(x[0], 2) * x[1]) / 2) + ((-6.4092722039791701 * pow(x[0], 2) * x[5]) / 2) + ((-6.4044892704425642 * pow(x[1], 2) * x[5]) / 2) + ((-1.7433342425888421 * pow(x[1], 2) * x[3]) / 2) + ((-0.53142479916341423 * pow(x[3], 3)) / 6) + ((-0.024950524189713351 * 0 * pow(x[0], 2)) / 2) + ((-0.0021314436129659439 * pow(x[2], 2) * x[3]) / 2) + ((-0.00045855276166194248 * pow(0, 2) * x[3]) / 2) + ((-0.00030993484769751645 * pow(0, 3)) / 6) + ((-9.6422026586060819e-05 * pow(0, 2) * x[3]) / 2) + ((-4.9387366140279154e-05 * x[3] * pow(x[5], 2)) / 2) + ((-3.7383833396900723e-05 * pow(x[3], 2) * x[5]) / 2) + ((-1.0763431740226098e-05 * pow(x[1], 2) * x[4]) / 2) + ((-6.4587701771726376e-06 * pow(x[0], 2) * x[2]) / 2) + ((-6.4587701771726376e-06 * pow(x[1], 2) * x[2]) / 2) + ((1.3190444673823373e-05 * pow(0, 2) * 0) / 2) + ((2.5399548740526985e-05 * pow(x[0], 2) * x[4]) / 2) + ((7.472953457774082e-05 * pow(x[5], 3)) / 6) + ((8.8009424710867011e-05 * 0 * pow(x[2], 2)) / 2) + ((0.00012372475833795745 * 0 * pow(x[5], 2)) / 2) + ((0.00017631162648910149 * x[1] * pow(x[4], 2)) / 2) + ((0.0033402252575842608 * 0 * pow(x[3], 2)) / 2) + ((0.0086970010215202988 * x[1] * pow(x[5], 2)) / 2) + ((0.010830936712579061 * 0 * pow(x[1], 2)) / 2) + ((0.015943550076935223 * x[1] * pow(x[2], 2)) / 2) + ((0.017268648546613752 * x[1] * pow(x[3], 2)) / 2) + ((0.018765305007514912 * pow(0, 2) * x[1]) / 2) + ((0.01938138365082779 * pow(0, 2) * x[1]) / 2) + ((0.036790575236646472 * pow(x[0], 2) * x[3]) / 2)),
        #                  ( - 0.028873066287985687 * 0 - 164.74437971669917 * x[0] + 0.41292628548056276 * x[2] - 7.1764113079695978e-06 * x[3] - 8.3713303835543655 * x[4] - 2.0090544711529489e-05 * x[5] - 9.5668727061472977e-05 * (0 * 0 * x[3]) + 2.4847157417661789e-05 * (0 * x[0] * x[2]) - 2.2362441675895609e-05 * (0 * x[0] * x[4]) + 0.032101927015986284 * (0 * x[0] * x[1]) + 2.4664946899649425e-05 * (0 * x[0] * x[3]) - 9.7788249678741122e-05 * (0 * x[2] * x[3]) + 0.00052618327399736251 * (x[0] * x[1] * x[2]) - 0.40377847274487111 * (x[0] * x[1] * x[3]) + 0.00016735939190385364 * (x[0] * x[1] * x[4]) + 7.1180776266778443 * (x[0] * x[1] * x[5]) - 0.015312948345974544 * (x[0] * x[2] * x[4]) + 0.015943550076935223 * (x[1] * x[2] * x[5]) - 0.00019590180721011281 * (x[1] * x[4] * x[5]) + ((-1.9918674673346259 * x[0] * pow(x[5], 2)) / 2) + ((-0.44558024172699939 * pow(x[0], 2) * x[2]) / 2) + ((-0.012564601684419098 * 0 * pow(x[1], 2)) / 2) + ((-0.0013655138220294348 * 0 * pow(x[2], 2)) / 2) + ((-9.7320608437667311e-05 * 0 * pow(x[5], 2)) / 2) + ((-9.5950246360772143e-05 * 0 * pow(x[3], 2)) / 2) + ((7.176411307969597e-06 * pow(x[1], 2) * x[3]) / 2) + ((1.1959368600251213e-05 * pow(x[1], 2) * x[5]) / 2) + ((1.1959368600251218e-05 * pow(x[0], 2) * x[5]) / 2) + ((1.4352822615939194e-05 * pow(x[0], 2) * x[3]) / 2) + ((0.00016025831394079408 * pow(0, 2) * x[2]) / 2) + ((0.00044870809209724837 * pow(0, 2) * x[2]) / 2) + ((0.0023682706810732711 * x[2] * pow(x[3], 2)) / 2) + ((0.01674088782171998 * x[0] * pow(x[2], 2)) / 2) + ((0.017880661999356982 * x[0] * pow(x[3], 2)) / 2) + ((0.031245422160302704 * pow(0, 2) * x[0]) / 2) + ((0.03168284188381567 * pow(0, 2) * x[0]) / 2) + ((0.056751613775557014 * 0 * pow(x[0], 2)) / 2) + ((0.063318353145585149 * x[0] * pow(x[4], 2)) / 2) + ((0.52201577053917037 * pow(x[2], 3)) / 6) + ((1.6822708208525212 * pow(x[1], 2) * x[2]) / 2) + ((8.3696402244800971 * pow(x[0], 2) * x[4]) / 2) + ((9.8792685238541189 * pow(x[1], 2) * x[4]) / 2) + ((535.59780832409683 * x[0] * pow(x[1], 2)) / 2) + ((690.40982114572751 * pow(x[0], 3)) / 6)),
        #                  ( - 0.032090488704376238 * 0 - 171.67340007243953 * x[1] - 7.1764113079695978e-06 * x[2] + 0.40410572991534277 * x[3] - 2.0090544711529489e-05 * x[4] - 7.1180776266778443 * x[5] + 9.7678247036896333e-06 * (0 * 0 * x[0]) - 9.5668727061472977e-05 * (0 * 0 * x[2]) + 0.00012970378400553901 * (0 * x[0] * x[1]) - 9.5950246360772129e-05 * (0 * x[2] * x[3]) + 0.00010813400937518589 * (0 * x[2] * x[5]) - 2.2198452209684476e-05 * (0 * x[0] * x[4]) + 0.00015274661523204623 * (0 * x[3] * x[5]) + 1.6778202130249511 * (x[0] * x[1] * x[2]) + 0.00056720485456160179 * (x[0] * x[1] * x[3]) + 9.8810740438233964 * (x[0] * x[1] * x[4]) + 2.0090544711529486e-05 * (x[0] * x[1] * x[5]) - 3.3094749262260714e-05 * (x[0] * x[2] * x[3]) - 0.016122381073757314 * (x[0] * x[3] * x[4]) + 2.0081325326653743 * (x[0] * x[4] * x[5]) + 0.017268648546613748 * (x[1] * x[3] * x[5]) + ((-0.021534870723141988 * pow(0, 2) * x[1]) / 2) + ((-0.02085033889723879 * pow(0, 2) * x[1]) / 2) + ((-0.019187387274015279 * x[1] * pow(x[3], 2)) / 2) + ((-0.017715055641039135 * x[1] * pow(x[2], 2)) / 2) + ((-0.0096633344683558862 * x[1] * pow(x[5], 2)) / 2) + ((-0.0043676840491391825 * 0 * pow(x[0], 2)) / 2) + ((-0.003711361397315845 * 0 * pow(x[3], 2)) / 2) + ((-0.00019590180721011275 * x[1] * pow(x[4], 2)) / 2) + ((-0.00013747195370884159 * 0 * pow(x[5], 2)) / 2) + ((-9.7788249678741108e-05 * 0 * pow(x[2], 2)) / 2) + ((-8.3032816197489804e-05 * pow(x[5], 3)) / 6) + ((-4.8312265534337242e-05 * pow(x[0], 2) * x[4]) / 2) + ((-1.4656049637581524e-05 * pow(0, 2) * 0) / 2) + ((-1.6940658945086007e-21 * pow(x[0], 2) * x[2]) / 2) + ((1.4352822615939194e-05 * pow(x[1], 2) * x[2]) / 2) + ((3.2049913311780707e-05 * pow(x[1], 2) * x[4]) / 2) + ((4.1537592663223022e-05 * pow(x[3], 2) * x[5]) / 2) + ((5.4874851266976834e-05 * x[3] * pow(x[5], 2)) / 2) + ((0.00010713558509562313 * pow(0, 2) * x[3]) / 2) + ((0.00034437205299724046 * pow(0, 3)) / 6) + ((0.0005095030685132694 * pow(0, 2) * x[3]) / 2) + ((0.0023682706810732707 * pow(x[2], 2) * x[3]) / 2) + ((0.0033359332990112733 * pow(x[0], 2) * x[5]) / 2) + ((0.020056114579288395 * 0 * pow(x[1], 2)) / 2) + ((0.36322731298573557 * pow(x[0], 2) * x[3]) / 2) + ((0.59047199907046022 * pow(x[3], 3)) / 6) + ((1.5329323174055929 * pow(x[1], 2) * x[3]) / 2) + ((14.23417681605847 * pow(x[1], 2) * x[5]) / 2) + ((356.00989156174745 * pow(x[0], 2) * x[1]) / 2) + ((1267.887182536733 * pow(x[1], 3)) / 6))])

        Taylor3_SDSOS_Q2e4 = np.array([x[2],
                         x[3],
                         ( - 0.028873066287985687 * 0 - 164.74437971669917 * x[0] + 0.41292628548056276 * 0 - 7.1764113079695978e-06 * 0 - 8.3713303835543655 * x[2] - 2.0090544711529489e-05 * x[3] - 9.5668727061472977e-05 * (0 * 0 * 0) + 2.4847157417661789e-05 * (0 * x[0] * 0) - 2.2362441675895609e-05 * (0 * x[0] * x[2]) + 0.032101927015986284 * (0 * x[0] * x[1]) + 2.4664946899649425e-05 * (0 * x[0] * 0) - 9.7788249678741122e-05 * (0 * 0 * 0) + 0.00052618327399736251 * (x[0] * x[1] * 0) - 0.40377847274487111 * (x[0] * x[1] * 0) + 0.00016735939190385364 * (x[0] * x[1] * x[2]) + 7.1180776266778443 * (x[0] * x[1] * x[3]) - 0.015312948345974544 * (x[0] * 0 * x[2]) + 0.015943550076935223 * (x[1] * 0 * x[3]) - 0.00019590180721011281 * (x[1] * x[2] * x[3]) + ((-1.9918674673346259 * x[0] * pow(x[3], 2)) / 2) + ((-0.44558024172699939 * pow(x[0], 2) * 0) / 2) + ((-0.012564601684419098 * 0 * pow(x[1], 2)) / 2) + ((-0.0013655138220294348 * 0 * pow(0, 2)) / 2) + ((-9.7320608437667311e-05 * 0 * pow(x[3], 2)) / 2) + ((-9.5950246360772143e-05 * 0 * pow(0, 2)) / 2) + ((7.176411307969597e-06 * pow(x[1], 2) * 0) / 2) + ((1.1959368600251213e-05 * pow(x[1], 2) * x[3]) / 2) + ((1.1959368600251218e-05 * pow(x[0], 2) * x[3]) / 2) + ((1.4352822615939194e-05 * pow(x[0], 2) * 0) / 2) + ((0.00016025831394079408 * pow(0, 2) * 0) / 2) + ((0.00044870809209724837 * pow(0, 2) * 0) / 2) + ((0.0023682706810732711 * 0 * pow(0, 2)) / 2) + ((0.01674088782171998 * x[0] * pow(0, 2)) / 2) + ((0.017880661999356982 * x[0] * pow(0, 2)) / 2) + ((0.031245422160302704 * pow(0, 2) * x[0]) / 2) + ((0.03168284188381567 * pow(0, 2) * x[0]) / 2) + ((0.056751613775557014 * 0 * pow(x[0], 2)) / 2) + ((0.063318353145585149 * x[0] * pow(x[2], 2)) / 2) + ((0.52201577053917037 * pow(0, 3)) / 6) + ((1.6822708208525212 * pow(x[1], 2) * 0) / 2) + ((8.3696402244800971 * pow(x[0], 2) * x[2]) / 2) + ((9.8792685238541189 * pow(x[1], 2) * x[2]) / 2) + ((535.59780832409683 * x[0] * pow(x[1], 2)) / 2) + ((690.40982114572751 * pow(x[0], 3)) / 6)),
                         ( - 0.032090488704376238 * 0 - 171.67340007243953 * x[1] - 7.1764113079695978e-06 * 0 + 0.40410572991534277 * 0 - 2.0090544711529489e-05 * x[2] - 7.1180776266778443 * x[3] + 9.7678247036896333e-06 * (0 * 0 * x[0]) - 9.5668727061472977e-05 * (0 * 0 * 0) + 0.00012970378400553901 * (0 * x[0] * x[1]) - 9.5950246360772129e-05 * (0 * 0 * 0) + 0.00010813400937518589 * (0 * 0 * x[3]) - 2.2198452209684476e-05 * (0 * x[0] * x[2]) + 0.00015274661523204623 * (0 * 0 * x[3]) + 1.6778202130249511 * (x[0] * x[1] * 0) + 0.00056720485456160179 * (x[0] * x[1] * 0) + 9.8810740438233964 * (x[0] * x[1] * x[2]) + 2.0090544711529486e-05 * (x[0] * x[1] * x[3]) - 3.3094749262260714e-05 * (x[0] * 0 * 0) - 0.016122381073757314 * (x[0] * 0 * x[2]) + 2.0081325326653743 * (x[0] * x[2] * x[3]) + 0.017268648546613748 * (x[1] * 0 * x[3]) + ((-0.021534870723141988 * pow(0, 2) * x[1]) / 2) + ((-0.02085033889723879 * pow(0, 2) * x[1]) / 2) + ((-0.019187387274015279 * x[1] * pow(0, 2)) / 2) + ((-0.017715055641039135 * x[1] * pow(0, 2)) / 2) + ((-0.0096633344683558862 * x[1] * pow(x[3], 2)) / 2) + ((-0.0043676840491391825 * 0 * pow(x[0], 2)) / 2) + ((-0.003711361397315845 * 0 * pow(0, 2)) / 2) + ((-0.00019590180721011275 * x[1] * pow(x[2], 2)) / 2) + ((-0.00013747195370884159 * 0 * pow(x[3], 2)) / 2) + ((-9.7788249678741108e-05 * 0 * pow(0, 2)) / 2) + ((-8.3032816197489804e-05 * pow(x[3], 3)) / 6) + ((-4.8312265534337242e-05 * pow(x[0], 2) * x[2]) / 2) + ((-1.4656049637581524e-05 * pow(0, 2) * 0) / 2) + ((-1.6940658945086007e-21 * pow(x[0], 2) * 0) / 2) + ((1.4352822615939194e-05 * pow(x[1], 2) * 0) / 2) + ((3.2049913311780707e-05 * pow(x[1], 2) * x[2]) / 2) + ((4.1537592663223022e-05 * pow(0, 2) * x[3]) / 2) + ((5.4874851266976834e-05 * 0 * pow(x[3], 2)) / 2) + ((0.00010713558509562313 * pow(0, 2) * 0) / 2) + ((0.00034437205299724046 * pow(0, 3)) / 6) + ((0.0005095030685132694 * pow(0, 2) * 0) / 2) + ((0.0023682706810732707 * pow(0, 2) * 0) / 2) + ((0.0033359332990112733 * pow(x[0], 2) * x[3]) / 2) + ((0.020056114579288395 * 0 * pow(x[1], 2)) / 2) + ((0.36322731298573557 * pow(x[0], 2) * 0) / 2) + ((0.59047199907046022 * pow(0, 3)) / 6) + ((1.5329323174055929 * pow(x[1], 2) * 0) / 2) + ((14.23417681605847 * pow(x[1], 2) * x[3]) / 2) + ((356.00989156174745 * pow(x[0], 2) * x[1]) / 2) + ((1267.887182536733 * pow(x[1], 3)) / 6))])
        return Taylor3_SDSOS_Q2e4
